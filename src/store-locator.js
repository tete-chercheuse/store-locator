...
export default class StoreLocator {
  ...
  constructor(options) {
    ...
    if(typeof this.options.functions.ready !== 'function') {
      throw new Error('store-locator :: No request function is set');
    }
  ...
  initMap() {
    ...
    let mapOptions = extend(true, {}, this.options.map.options)
    ...
    if(typeof window.MarkerClusterer !== 'undefined') {
      if(this.options.cluster.status) {
        // Clone object before to prevent reference
        let cloneClusterOptions = extend(
          true,
    ...
    if(typeof this.options.functions.ready === 'function') {
      this.options.functions.ready(this.map);
    }
  }
