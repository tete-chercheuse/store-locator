import { setWorkerUrl, getWorkerUrl, getVersion, Map as Map$1, NavigationControl, GeolocateControl, Marker, Popup } from 'maplibre-gl';
...
/**
 * Feuille de style de MapLibre GL JS 6.9.0, embarquée.
 *
 ...
 */
var MARQUEUR = 'data-store-locator-css';
var injectMapLibreCss = function injectMapLibreCss(nonce) {
...
var PIXEL_TRANSPARENT = {
  width: 1,
  height: 1,
  data: new Uint8Array(4)
};
var resolveMissingImages = function resolveMissingImages(map, collecte) {
 ...
 */
var BUNDLED_WORKER_VERSION = '6.9.0';
...
var mineure = function mineure(version) {
  return version.split('.').slice(0, 2).join('.');
};
var avertir = function avertir(message) {
 ...
 */
var configureWorker = function configureWorker(workerUrl) {
  if (workerUrl) {
  ...
  if (mineure(installee) !== mineure(BUNDLED_WORKER_VERSION)) {
    avertir("Le worker embarqu\xE9 vient de maplibre-gl " + BUNDLED_WORKER_VERSION + ", or " + installee + " est install\xE9e. " + 'Il n\'est pas utilisé : le protocole entre le worker et le thread principal est interne à MapLibre. ' + ("Aligne maplibre-gl sur " + mineure(BUNDLED_WORKER_VERSION) + ".x, ou passe map.workerUrl."));
    return;
  }
...
var createMap = function createMap(container, config, unresolvedImages) {
 ...
 */
var iconSize = function iconSize(value) {
 ...
 */
var anchorPoint = function anchorPoint(_ref, anchor) {
  var width = _ref[0],
    height = _ref[1];
  var x = anchor.includes('left') ? 0 : anchor.includes('right') ? width : width / 2;
  var y = anchor.includes('top') ? 0 : anchor.includes('bottom') ? height : height / 2;
  return [x, y];
};
/**
 ...
 */
var popupOffsetForIcon = function popupOffsetForIcon(size, anchor) {
  if (!size) {
    return null;
  }
  var width = size[0],
    height = size[1];
  var _anchorPoint = anchorPoint(size, anchor != null ? anchor : 'center'),
    ax = _anchorPoint[0],
    ay = _anchorPoint[1];
...
var MarkerSync = /*#__PURE__*/function () {
  ...
  _proto.createMarker = function createMarker(feature) {
    var _this2 = this;
    var iconValue = this.resolveIcon(feature);
    var iconOptions = normalizeIcon(iconValue);
    ...
    if (popup) {
      ...
      // reste prioritaire.
      var offset = 'offset' in popup.options ? undefined : popupOffsetForIcon(iconSize(iconValue), iconOptions == null ? void 0 : iconOptions.anchor);
      var instance = new Popup(offset ? _extends({}, popup.options, {
        offset: offset
      }) : popup.options);
...
var StoreLocator = /*#__PURE__*/function () {
  ...
  return _createClass(StoreLocator, [{
     ...
     */
    function get() {
      return [].concat(this.missingImages);
    }
    /**
...
export { OPENFREEMAP_BRIGHT, StoreLocator as default };
