 ...
 */
const BUNDLED_WORKER_VERSION = '6.9.0';
...
const mineure = version => version.split('.').slice(0, 2).join('.');
 ...
 */
const configureWorker = workerUrl => {
  if (workerUrl) {
  ...
  if (mineure(installee) !== mineure(BUNDLED_WORKER_VERSION)) {
    avertir(`Le worker embarqué vient de maplibre-gl ${BUNDLED_WORKER_VERSION}, or ${installee} est installée. ` + 'Il n\'est pas utilisé : le protocole entre le worker et le thread principal est interne à MapLibre. ' + `Aligne maplibre-gl sur ${mineure(BUNDLED_WORKER_VERSION)}.x, ou passe map.workerUrl.`);
    return;
  }
